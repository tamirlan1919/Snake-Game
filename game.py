...
class game():
    # sound = pygame.mixer.Sound(path.join(snd_dir, 'pew.wav'))
    x = 1
    scoring_ = 1
    code_ = 0
    RES = 700
    ...
    def start(self):
        ...
        while True:
            ...
            position = font_score.render(f'x: {x} y: {y}', 1, pygame.Color('blue'))
            time = font_score.render(f'TIME: {time_}', 1, pygame.Color('black'))
            help_ = font_score.render('w: вверх       '
                                      'a: влево       '
                                      's: вниз       '
                                      'd: вправо       '
                                      'r: заново       ', 1, pygame.Color('white'))
            ...
            if x < 0 or x > self.RES - self.SIZE or y < 0 or y > self.RES - self.SIZE or len(snake) != len(set(snake)):
                render_end = font_end.render('GAME OVER ! ', 1, pygame.Color('cyan'))
